...
const sortMeetingsByDay = (arr) => {
  ...
  return arr.sort((a,b) => weekArray.indexOf(a.dayOfWeek) < weekArray.indexOf(b.dayOfWeek) ? -1 : 1);
};
...
const sortSchedule = (arr) => {
  const weekArray = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'];
  return arr.sort((a,b) => {
    if(weekArray.indexOf(a.dayOfWeek) === weekArray.indexOf(b.dayOfWeek)) {
      if(a.start === b.start){
      ...
      return weekArray.indexOf(a.dayOfWeek) < weekArray.indexOf(b.dayOfWeek);
    }
  });
};
