...
const addQuestion = (arr) => arr.map( something => `${something}?`);
...
const forEachTwoToThe = (arr) => {
  const cheetos = [];
  arr.forEach( number => cheetos.push(Math.pow(2, number)));
  return cheetos;
};
...
const charCode = (arr) => arr.map( (value, index) => arr.join('').charCodeAt(index));
